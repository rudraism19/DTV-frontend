import { CheckCircle2, Circle, Clock, Target, TrendingUp, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="mb-8">
        ...
        <p className="text-blue-200">Track high-level engagement and daily execution</p>
      </div>

      {/* Top Metrics Row */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass-panel p-6 border-blue-500/30">
          <h3 className="text-sm font-medium text-blue-300 mb-1">Accuracy Measure</h3>
          <div className="flex items-end gap-3">
            <span className="text-4xl font-bold text-white tracking-tighter">88%</span>
            <span className="text-sm text-green-400 flex items-center mb-1"><TrendingUp size={16} className="mr-1"/> +5%</span>
          </div>
          <p className="text-xs text-text-muted mt-2">Goals completed vs planned</p>
        </div>

        <div className="glass-panel p-6 lg:col-span-3 border-orange-500/30 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/10 rounded-full blur-[40px] group-hover:bg-orange-500/20 transition-all pointer-events-none"></div>
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <p className="text-sm text-text-muted">This will boost Career Matching by 15% for Software Architect role.</p>
            </div>
            <button 
              onClick={() => setShowRoadmapModal(true)}
              className="px-4 py-2 bg-orange-500/20 hover:bg-orange-500/30 text-orange-300 rounded-lg text-sm font-bold transition-all transform hover:scale-105 active:scale-95 shadow-[0_4px_10px_rgba(249,115,22,0.2)]"
            >
              View Roadmap
            </button>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyData}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ color: '#fff' }}
                />
                <Area type="monotone" dataKey="focus" stroke="#3b82f6" fillOpacity={1} fill="url(#colorFocus)" name="Focus Level" />
                <Area type="monotone" dataKey="goalCompletion" stroke="#10b981" fillOpacity={1} fill="url(#colorGoals)" name="Goals Hit" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {routine.map((item, idx) => (
              <div key={idx} className="flex gap-4 relative">
                {idx !== routine.length - 1 && (
                  <div className="absolute left-2.5 top-6 bottom-[-20px] w-px bg-white/10"></div>
                )}
                <div className="flex-shrink-0 mt-1">
                  {item.status === 'completed' ? (
                    <CheckCircle2 size={20} className="text-green-400 bg-bg rounded-full" />
                  ) : item.status === 'in-progress' ? (
                    <Circle size={20} className="text-blue-400 fill-blue-400/20 bg-bg rounded-full" />
                  ) : (
