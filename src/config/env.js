...
const schema = Joi.object({
  ...
  DB_POOL_MAX: Joi.number().default(15),
  DB_IDLE_TIMEOUT_MS: Joi.number().default(10000),
  DB_CONNECTION_TIMEOUT_MS: Joi.number().default(5000),
  DB_SSL_OFF: Joi.boolean().truthy('true').falsy('false').default(false),
  SUPABASE_URL: Joi.string().allow(''),
  SUPABASE_ANON_KEY: Joi.string().allow(''),
  ...
  AI_PROVIDER: Joi.string().lowercase().valid('auto', 'anthropic', 'openai', 'gemini', 'groq').default('auto'),
